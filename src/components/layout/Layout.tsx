      ...
      <header className="flex-shrink-0 flex items-center justify-between px-4 sm:px-6 h-[64px] border-b border-border bg-card backdrop-blur-md z-40">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
            ...
            {navItems.map(({ path, label, icon: Icon }) => {
              ...
              return (
                <Link
                  key={path}
                  to={path}
                  className={`flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3.5 py-1.5 sm:py-2 rounded-lg text-[10px] sm:text-xs md:text-sm font-bold tracking-wide transition-all ${
                    isActive
          ...
          <button
            ...
            className="rounded-full p-2 text-text-secondary hover:bg-button-secondary hover:text-text-primary transition-all active:scale-95"
            title="Open Settings"
          >
